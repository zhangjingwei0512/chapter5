...
with open('further2.csv','w',newline='')as f:
    writer=csv.writer(f)
    header=['month','detail']
    ...
    for i in range(1,9):
        for j in range(1,13):
            if i==8 and j>9:
                ...
                url='https://github.com/myersjustinc?tab=overview&from=201{year}-{month}-01&to=201{year}-{month}-31'.format(year=i,month=j)
                r=requests.get(url).text
                data=BeautifulSoup(r,"html.parser")
                month_1='201{0}/{1}'.format(i,j)
